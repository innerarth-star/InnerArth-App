...
export default function TabLayout() {
  ...
  return (
    <Tabs screenOptions={{ 
      ...
      {/* 1. MI PLAN: El Coach lo ve pero el Cliente también. 
          Para que el Admin NO tenga el espacio vacío, no usamos null en el botón */}
      <Tabs.Screen
        name="index"
        options={{
          title: 'Mi Plan',
          tabBarIcon: ({ color }) => <FontAwesome5 name="clipboard-list" size={20} color={color} />,
          // El Coach lo ve para que no haya huecos, el Cliente también lo ve
        }}
      />

      {/* 2. CLIENTES: Solo para el Coach */}
      <Tabs.Screen
        name="coach"
        options={{
          title: 'Clientes',
          // Ocultamos visualmente para el cliente sin romper el render
          tabBarItemStyle: !isCoach ? { display: 'none' } : {},
          tabBarIcon: ({ color }) => <FontAwesome5 name="users" size={20} color={color} />,
        }}
      />

      {/* 3. BIBLIOTECA: Solo para el Coach */}
      <Tabs.Screen
        name="AdminAlimnetos"
        options={{
          title: 'Biblioteca',
          // Ocultamos visualmente para el cliente
          tabBarItemStyle: !isCoach ? { display: 'none' } : {},
