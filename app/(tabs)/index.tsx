...
function ClienteScreen({ user }: { user: any }) {
  ...
  const enviarAlCoach = async () => {
    if (!nombre || !firma || !aceptarTerminos || !aceptarPrivacidad || !datosFisicos.edad) {
      Alert.alert("Atención", "Por favor completa todos los campos (Nombre, Edad, Firma y Avisos).");
      return;
    }
    try {
      ...
      setPaso('espera');
    } catch (e) { Alert.alert("Error", "Error al conectar con la base de datos."); }
  };
    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        <Section num={6} title="Nutrición y Hábitos" color="#8b5cf6" icon="utensils" activa={seccionActiva} setActiva={setSeccionActiva}>
          <Text style={styles.labelSub}>¿Cuántas comidas haces actualmente?</Text>
          ...
          <TextInput style={styles.input} placeholder="Describe un día de tus comidas" onChangeText={v=>setNutricion({...nutricion, descripcionDia:v})} />
          ...
          <Text style={[styles.labelSub, {marginTop:10}]}>¿Sustancias o fuma?</Text>
          ...
          <Text style={[styles.labelSub, {marginTop:10}]}>¿Cuántas comidas deseas?</Text>
          ...
          <TextInput style={styles.input} placeholder="Describe tus objetivos deseados" onChangeText={setObjetivo} />
        ...
        <Section num={9} title="Aviso de Privacidad" color="#64748b" icon="shield-alt" activa={seccionActiva} setActiva={setSeccionActiva}>
          <View style={styles.consentBox}>
            ...
            <Text style={styles.consentTxt}>FitTech, con domicilio para oír y recibir notificaciones, es el responsable del uso y protección de sus datos personales. Sus datos serán utilizados para: Proveer los servicios de asesoría deportiva y nutricional contratados; Integrar su expediente clínico y deportivo; Contactarle para dar seguimiento a su plan. Usted tiene derecho a ejercer sus derechos ARCO (Acceso, Rectificación, Cancelación y Oposición) a través del Coach Arturo.</Text>
          </View>
